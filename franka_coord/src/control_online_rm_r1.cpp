...
void start_move(franka_coord::TrajPara msg)
{
    ...
    if (change_goal)
    {
        ...
        motion_generation.ReInitialize2(state_pub,speed_factor,q_goal_t,slow_factor);
        ...
        {                 
            ...
            for (size_t i = 0; i < 7; i++)
            {
                if (std::abs(robot_state.q_d[i] - msg.qgoal[i]) > 10e-6)
                {
                    reach_goal = 0;
                }
            }
...
{
    ros::init(argc, argv, "control_online_r1");
    ros::NodeHandle node_handle;

    motion_generation.control_mode = 2;

    ROS_INFO("Waiting for traj to move R1...");
    ros::ServiceServer service = node_handle.advertiseService("temp_goal_r1", update_goal);
