...
void start_move(franka_coord::TrajPara msg)
{
    ...
    if (change_goal)
    {
        ROS_INFO("Got new traj: R%d", msg.robotid);

        speed_factor = msg.speedfactor;
        time_start = msg.starttime;
        std::array<double, 7> q_goal_t;
        for (size_t i = 0; i < 7; i++)
        {
            q_goal[i] = msg.qgoal[i];
            q_goal_t[i] = msg.qgoalt[i];
        } 

        motion_generation.ReInitialize(speed_factor,q_goal_t,slow_factor);
        ...
        while (!reach_goal) //--comment for testing initial temp goal
        {
            ...
            while(!motion_finished && ros::ok())
            {
                ...
                motion_finished=motion_generation.generateMotionOnline(sample_time);
...
bool update_goal(franka_coord::SendTrajPara::Request  &req,
                 franka_coord::SendTrajPara::Response &res)
{
    ROS_INFO("R%d temporary goal changed.", req.robotid);

    std::array<double, 7> q_goal_t;
    for (size_t i = 0; i < 7; i++)
    {
        q_goal_t[i] = req.qgoal[i];
    }
    motion_generation.ReInitialize(speed_factor,q_goal_t,slow_factor); 
...
{
    ros::init(argc, argv, "control_online");
    ros::NodeHandle node_handle;
    ...
    ros::ServiceServer service = node_handle.advertiseService("temp_goal_r"+std::to_string(robot_id), update_goal);
